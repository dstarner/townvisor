...
const FeedItem = ({
  ...
  return (
    <Card
      ...
      style={matches ? {flexDirection: 'column'} : {}}
    >
      {imgURL && imgURL.length > 0 ? (
        <CardMedia
          className={classes.cover}
          style={
            matches ? {height: '151px', flex: 'inherit', width: 'inherit'} : {}
          }
          image={imgURL}
          title={title}
        />
      ...
      <div className={classes.details}>
        <CardContent className={classes.content}>
          ...
          {author ? (
            <Typography variant="subtitle1" color="textSecondary">
              By{' '}
              <Link className={classes.author} href={`${author.profileUrl}`}>
                @{author.username}
              </Link>
            </Typography>
          ) : null}
        ...
        <div className={classes.statistics}>
          ...
          {views ? (
            <Typography variant="subtitle1" color="textSecondary">
              {views < 25 ? 'Less than 25' : views} views
            </Typography>
          ) : null}
        </div>
      </div>
